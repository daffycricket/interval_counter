...
- Configuration in `l10n.yaml` (template: `app_fr.arb`, output: `app_localizations.dart`)
- Localization files are auto-generated by Flutter during build
- To regenerate manually: `flutter gen-l10n` (usually not needed)
...
Shared test utilities are located in `test/helpers/`:
- `widget_test_helpers.dart` - Common setup functions for widget tests
- Use `createMockState()`, `createTestApp()` to reduce boilerplate
- All widget tests should import and use these helpers
...
This project is in active development. The main screen (IntervalTimerHomeScreen) is being built. Key generated files:
- `lib/main.dart` - App entry point with Provider setup
- `lib/state/interval_timer_home_state.dart` - Home screen state with Preset model
- `lib/screens/interval_timer_home_screen.dart` - Home screen widget
- `lib/widgets/value_control.dart` - Reusable value control widget with +/- buttons
- `lib/widgets/interval_timer_home/` - Screen-specific widgets (presets section, quick start, etc.)
- `lib/theme/` - Design system tokens (colors, text styles)
- `lib/l10n/` - Localization files (English and French)
- `test/` - Comprehensive test suite mirroring lib/ structure

The project follows an iterative generation workflow with source files in `snapshot2app_agent_context/sources/`.
