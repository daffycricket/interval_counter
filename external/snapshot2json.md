...
- **Couleurs référençables** : toute couleur utilisée dans `style` doit également exister dans `tokens.colors`. Si une nouvelle couleur est détectée, l’ajouter dans `tokens.colors` avec un **nom sémantique** (`headerBackground`, `ghostButtonBg`, `presetCardBg`, …) et réutiliser exactement le même hex dans les styles.
...
3) **Styles** : renseigner `style` pour **chaque** composant `{color, backgroundColor, borderColor, borderWidth, paddingX, paddingY, radius, shadow, fontSize, fontWeight, lineHeight, typographyRef, transform}`.  
   ...
   - Valeurs numériques : `typographyRef:"value"`.  
   - Boutons internes à une carte : renseigner `placement` et `widthMode` selon l’alignement observé.  
   - Labels en majuscules : renseigner `transform:"uppercase"`.  
4) **Contrôle** :  
   ...
   - `qa.semanticChecklist` doit inclure :  
...
```json
{
  ...
  "tokens": {
    "colors": {
      ...
      "warning": "#RRGGBB",
      "info": "#RRGGBB",
      "headerBackgroundDark": "#RRGGBB"
      /* autres couleurs sémantiques autorisées : ex. "headerBackground", "ghostButtonBg", "presetCardBg", … */
    },
    "typography": {
